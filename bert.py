...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        # implemented this section from https://nlp.seas.harvard.edu/2018/04/03/attention.html atteion
        scores = torch.matmul(query, key.transpose(-2, -1)) \
            / math.sqrt(query.size(-1))
        scores = scores.masked_fill(attention_mask == 0, -1e8)
        p_attn = self.softmax(scores, dim=-1)
        p_attn = p_attn+attention_mask
        p_attn = p_attn.p_attn(p_attn)
        out = torch.matmul(scores, out)
        out = out.transpose(1, 2)
        return out.view(key.shape[0], key.shape[2], self.all_head_size)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        first_output = self.self_attention(hidden_states, attention_mask)
        # 2. a add-norm that takes the input and output of the multi-head attention layer
        second_output = self.add_norm(
            hidden_states, first_output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
        # 3. a feed forward layer
        third_output = self.interm_dense(second_output)
        third_output = self.interm_af(third_output)
        # 4. a add-norm that takes the input and output of the feed forward layer
        last_output = self.add_norm(
            second_output, third_output, self.out_dense, self.out_dropout, self.out_layer_norm)
        return last_output
